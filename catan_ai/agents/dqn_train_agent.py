...
class DQNTrainAgent(Player):
    ...
    def game_over(self, game: Game) -> None:
        ...
        enemy_vp = sum(
            get_visible_victory_points(game.state, enemy.color)
            for enemy in game.state.players
            if enemy.color != self.color
        )

        game_result_reward = 100 if player_vp >= 10 else -100 if enemy_vp >= 10 else 0
    ...
    def play_turn_decide(self, game: Game, playable_actions: Iterable[Action]) -> Action:
        ...
        if action_idx == 0:
            action = Action(self.color, ActionType.END_TURN, None)
            reward = 0
        elif action_idx < NUM_NODES + 1:
            if obs[action_idx - 1] == 1:
                action = Action(self.color, ActionType.BUILD_CITY, action_idx - 1)
                reward = 1
            else:
                action = Action(self.color, ActionType.BUILD_SETTLEMENT, action_idx - 1)
                reward = 1
        else:
            action = Action(self.color, ActionType.BUILD_ROAD, action_idx + NUM_NODES - 1)
            reward = 0.1
